...
Each example links the kit via the pnpm workspace (`workspace:*`), so build the library first, then
run an example:
...
`examples/server` is a standalone Node example (npm, `file:` link) and is not part of the workspace.
