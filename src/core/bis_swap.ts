...
import { add_liquidity_request, remove_liquidity_request, save_info, swap2_request, swap_request, unwrap_request, withdraw_request } from '../lib/uniswap_ops'
import { bitcoinjs, getNetwork } from '../main'
import { compressSmartContractData } from './brc20'
import { clear_extra_utxos, save_extra_utxos, utxo_output_type_from_outputScript } from './helpers'
import { InscriptionDetails, mint_all, mint_all_check_fees, mint_with_extra_input_in_commit_all, mint_with_extra_input_in_commit_fee_rate, send_inscription_to_op_return_with_extra_inputs_and_extra_output_all, send_inscription_to_op_return_with_extra_inputs_and_extra_output_fee_rate, WalletInfo } from './mint'
import { getOrdinalsWallet, getPaymentWallet, getSignFn, signMessageLocalVerify, signMessageLocalVerifyDeterministic } from './providers'
import { getWalletInfo, readSwapWalletInfo, saveSwapWalletInfo } from './store'

const abi_signet = [{ type: 'function', name: 'deposit', inputs: [{ name: 'tokenAddress', type: 'address', internalType: 'address' }, { name: 'tickerIdx', type: 'uint32', internalType: 'uint32' }, { name: 'amt', type: 'uint256', internalType: 'uint256' }, { name: 'pubkey', type: 'bytes', internalType: 'bytes' }, { name: 'pkIdx', type: 'uint64', internalType: 'uint64' }, { name: 'negSignatureBLS12', type: 'bytes', internalType: 'bytes' }, { name: 'ecSignatureForIndexes', type: 'bytes', internalType: 'bytes' }], outputs: [], stateMutability: 'nonpayable' }]
const abi_mainnet = abi_signet

const dummy_utxo_value = 1000
const GAS_PER_BYTE = 12000

// TODO: Update the text below if needed
const signatureRequestTextSignet = `Sign this message only on official BiS interfaces.
...
const signatureRequestTextMainnet = `Sign this message only on official BiS interfaces.
This signature is required to generate your session-specific swap key.
Signing this message outside official BiS interfaces may result in unauthorized access to your swap session and is at the user's own risk.
By signing, you acknowledge that the BiS Terms of Service also apply: https://bestinslot.xyz/legal/terms.
...
function getSignatureRequestText(): string {
  ...
  if (network === 'signet') {
    return signatureRequestTextSignet
  }
  else if (network === 'mainnet') {
    return signatureRequestTextMainnet
  }
  else {
...
function getSwapContractInterface(): ethers.Interface {
  const network = getNetwork()
  if (network === 'signet') {
    return new ethers.Interface(abi_signet)
  }
  else if (network === 'mainnet') {
    return new ethers.Interface(abi_mainnet)
  }
  else {
    throw new Error('Unsupported network for orderbook contract address')
  }
...
export async function generateAndStoreSwapWallet(): Promise<SwapWalletInfo> {
  ...
  const signature_hex = await signMessageLocalVerifyDeterministic(getSignatureRequestText())
  await new Promise(resolve => setTimeout(resolve, 500))
  const signature_hex_twin = await signMessageLocalVerifyDeterministic(getSignatureRequestText())
  if (signature_hex !== signature_hex_twin) {
  ...
  const bls_privkey_ikm = Buffer.from(ethers.keccak256(`0x${signature_hex}`).slice(2), 'hex')

  // Expand your input key material to 48 bytes.
  // For a 32-byte scalar, 48 bytes of hash gives ~2^-128 bias, per FIPS 186-5 / RFC 9380.
  const info_buffer = Buffer.from('my-app/bls-keygen', 'utf-8')
  const okm = hkdf(sha256, bls_privkey_ikm, undefined, info_buffer, 48)

  // Map to scalar in [0, r-1], where r is BLS12-381 Fr order
  const skScalar = mod.mapHashToField(okm, bls12_381.fields.Fr.ORDER)
  const bls_privkey = Buffer.from(skScalar)

  const bls_pubkey = bls12_381.shortSignatures.getPublicKey(bls_privkey)
  const bls_pubkey_hex = `0x${bls_pubkey.x.c0.toString(16).padStart(128, '0')}${bls_pubkey.x.c1.toString(16).padStart(128, '0')}${bls_pubkey.y.c0.toString(16).padStart(128, '0')}${bls_pubkey.y.c1.toString(16).padStart(128, '0')}`
  const bls_privkey_hex = `0x${bls_privkey.toString('hex')}`

  // 3. Create Swap Wallet Info and store in DB
  const swapWalletInfo: SwapWalletInfo = {
    bitcoinAddress: inscriptionWallet.address,
    swapPubkey: bls_pubkey_hex,
    swapPrivkey: bls_privkey_hex,
  }
...
export async function checkSwapAllowance(token_address: string): Promise<bigint> {
  // 1. Validate Wallet
  const inscriptionWallet = getOrdinalsWallet()
  if (!inscriptionWallet?.address) {
    throw new Error('Ordinals wallet address not found.')
...
let swap_info_cache: SwapInfo | null = null
async function getSwapInfo(): Promise<SwapInfo> {
  if (swap_info_cache) {
    return swap_info_cache
  }
...
export async function checkBRC2_0Balance(token_address: string): Promise<bigint> {
  // 1. Validate Wallet
  const inscriptionWallet = getOrdinalsWallet()
  if (!inscriptionWallet?.address) {
    throw new Error('Ordinals wallet address not found.')
...
interface CheckBaseBRC20BalanceResponse {
  success: boolean
  result: {
    available_balance_in_18_dec: string
    transferrable_balance_in_18_dec: string
    decimals: number
    ticker: string
  }
}
export async function checkBaseBRC20Balance(token_address: string): Promise<{ available_balance_in_18_dec: bigint, transferrable_balance_in_18_dec: bigint, decimals: number, ticker: string }> {
  // 1. Validate Wallet
  const inscriptionWallet = getOrdinalsWallet()
  if (!inscriptionWallet?.address) {
    throw new Error('Ordinals wallet address not found.')
...
export async function checkBRC2_0BalanceOfPaymentWallet(token_address: string): Promise<bigint> {
  // 1. Validate Wallet
...
interface API_GetAllBalancesResponse {
  success: boolean
  result: CheckSwapBalancesItem[]
}

export async function checkSwapBalances(ordinal_address: string): Promise<CheckSwapBalancesItem[]> {
  // 2. Prepare and execute the API call
  ...
  const result = await fetchWithErrors<API_GetAllBalancesResponse>(url, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
export async function checkSwapBalance(token_address: string): Promise<bigint> {
  // 1. Validate Wallet
...
async function checkSwapBalanceOf(pubkey: string, token_address: string): Promise<bigint> {
  // 2. Prepare and execute the API call
  const url = getSwapBackendUrl('get_balance')
  const body = {
    pubkey,
...
export async function checkPairReserves(pair_address: string): Promise<PairReserves> {
  // 2. Prepare and execute the API call
...
export async function requestMinerFee(type: 'add_liquidity' | 'remove_liquidity' | 'swap' | 'withdraw' | 'unwrap'): Promise<bigint> {
  // 2. Prepare and execute the API call
...
export async function getPairVolumeOverDays(params: GetPairVolumeRequest): Promise<GetPairVolumeResponse> {
  // 2. Prepare and execute the API call
...
export async function getKlines(params: GetKlinesRequest): Promise<GetKlinesResponse> {
  ...
  // 2. Prepare and execute the API call
  let url = getSwapBackendUrl(`klines/${params.pair_address}?interval=${params.interval}&limit=${params.limit}`)
...
export async function getActivityOfPair(params: GetActivityOfPairRequest): Promise<GetActivityOfPairResponse> {
  if (params.limit > 200) {
    throw new Error('Limit cannot exceed 200')
  }

  // 2. Prepare and execute the API call
  const url = getSwapBackendUrl(`pair-activity/${params.pair_address}?limit=${params.limit}&offset=${params.offset}`)
...
export interface WalletActivityEntry {
  id: string
  type: 'add_liq' | 'remove_liq' | 'swap1' | 'swap2' | 'deposit' | 'unwrap' | 'withdraw' | 'withdraw (lp)'
...
export async function getWalletActivities(params: GetWalletActivitiesRequest): Promise<GetWalletActivitiesResponse> {
  // 2. Prepare and execute the API call
  const url = getSwapBackendUrl(`wallet-activity/${params.pubkey}?pairAddress=${params.pairAddress}`)
...
export async function getTokenDecimals(token_address: string): Promise<number> {
  // 2. Prepare and execute the API call
...
async function getDepositBIP322Signature(to_send: DepositBIP322SignatureRequest): Promise<string> {
  to_send.bls_pubkey = to_send.bls_pubkey.startsWith('0x') ? to_send.bls_pubkey.slice(2) : to_send.bls_pubkey
  to_send.bls_pubkey = to_send.bls_pubkey.toLowerCase()
  to_send.token_address = to_send.token_address.toLowerCase()

  const signature_text = `Deposit Order:
Bitcoin Address: ${to_send.btc_address}
BLS Public Key: ${to_send.bls_pubkey}
Token Address: ${to_send.token_address}
...
async function getDepositBLSSignature(params: DepositBLSSignatureRequest): Promise<string> {
  ...
  const bls_private_key = swapWallet.swapPrivkey.startsWith('0x') ? swapWallet.swapPrivkey.slice(2) : swapWallet.swapPrivkey
  ...
  const negatedSig = bls_signature.negate()
...
async function getDepositSignature(to_send: DepositSignatureRequest): Promise<DepositSignatureResponse> {
...
async function broadcastDepositOrder(to_send: BroadcastDepositOrderRequest): Promise<BroadcastDepositOrderResponse> {
...
function convertAmountToBRC20String(amount_in_dec_18: bigint, decimals: number): string {
  if (amount_in_dec_18 < 0n) {
  ...
  const amountStr = amount_in_dec_18.toString().padStart(amt_dec + 1, '0')
  const integerPart = amountStr.slice(0, -amt_dec)
  let fractionalPart = amountStr.slice(-amt_dec).replace(/0+$/, '') // remove trailing zeros
...
export async function createAndBroadcastDepositOrder(
  ...
  const current_token_amount = await checkBRC2_0Balance(token_address)
  if (current_token_amount < token_amount) {
    const current_base_available_token_info = await checkBaseBRC20Balance(token_address)
    const current_base_available_token_amount = current_base_available_token_info.available_balance_in_18_dec
    ...
    if (current_base_available_token_amount < use_base_available_balance_amt) {
      console.error(`Insufficient BRC-2.0 + BRC20 available balance. Current BRC-2.0: ${current_token_amount}, Available in base: ${current_base_available_token_amount}, Required: ${token_amount}`)
  ...
  if (current_allowance < BigInt('0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff')) {
  ...
  const allowance_content = Buff.str(`{"p":"brc20-prog","op":"c","c":"${token_address}","b":"${allowance_calldata_compressed}"}`)
  ...
  const deposit_content = Buff.str(`{"p":"brc20-prog","op":"c","c":"${l1_contract_address}","b":"${deposit_calldata_compressed}"}`)
  ...
  let base_deposit_inscription_id: string | null = null
  let base_deposit_secret: string | null = null
  let base_deposit_commit_txid: string | null = null
  let base_deposit_reveal_txid: string | null = null
  let base_deposit_commit_tx_hex: string | null = null
  let base_deposit_reveal_tx_hex: string | null = null
  let base_deposit_send_to_op_return_tx_hex: string | null = null
  let base_deposit_send_to_op_return_txid: string | null = null
  let base_deposit_mint_res = null
  if (use_base_available_balance_amt > 0n) {
    const amount_to_deposit = convertAmountToBRC20String(use_base_available_balance_amt, base_token_decimals)
    const base_deposit_inscription_details = new InscriptionDetails(
      ...
      null,
      null,
      Buff.str(`{"p":"brc-20","op":"transfer","tick":"${base_token_ticker}","amt":"${amount_to_deposit}"}`),
    )

    base_deposit_mint_res = await mint_all(base_deposit_inscription_details, fee_rate, null, null, 0, true, signFn)
    base_deposit_commit_tx_hex = base_deposit_mint_res.signed_commit_tx_hex
    base_deposit_reveal_tx_hex = base_deposit_mint_res.signed_reveal_tx_hex
    base_deposit_commit_txid = base_deposit_mint_res.commit_txid
    base_deposit_reveal_txid = base_deposit_mint_res.reveal_txid
    base_deposit_inscription_id = base_deposit_mint_res.inscription_id
    const satpoint = `${base_deposit_inscription_id.split('i')[0]}:0:0`
    base_deposit_secret = base_deposit_mint_res.secret
    let send_to_opreturn_res = null
    try {
      if (!base_deposit_commit_tx_hex) {
        throw new Error('Base deposit commit tx hex is missing')
      }
      save_extra_utxos([base_deposit_commit_tx_hex, base_deposit_reveal_tx_hex], [base_deposit_inscription_id, satpoint])
      ...
      send_to_opreturn_res = await send_inscription_to_op_return_with_extra_inputs_and_extra_output_all(base_deposit_inscription_id, [], target_wallet, 1, extra_outputs, fee_rate, true, signFn)
    }
    finally {
      clear_extra_utxos()
    }
    ...
    base_deposit_send_to_op_return_tx_hex = send_to_opreturn_res.signed_tx_hex
  ...
  if (needs_allowance) {
    ...
    if (use_base_available_balance_amt > 0n) {
      extra_txhexes_for_allowance.push(base_deposit_commit_tx_hex!, base_deposit_reveal_tx_hex!, base_deposit_send_to_op_return_tx_hex!)
    }
    try {
      save_extra_utxos(extra_txhexes_for_allowance, null)
      allowance_mint_res = await mint_all(allowance_inscription_details, fee_rate, null, null, 0, true, signFn)
    ...
    try {
      ...
      save_extra_utxos(extra_txhexes_for_allowance, [allowance_inscription_id, satpoint])
      ...
      send_to_opreturn_res = await send_inscription_to_op_return_with_extra_inputs_and_extra_output_all(allowance_inscription_id, [], target_wallet, 1, extra_outputs, fee_rate, true, signFn)
    }
    finally {
      clear_extra_utxos()
    }
  ...
  if (use_base_available_balance_amt > 0n) {
    if (!base_deposit_commit_tx_hex || !base_deposit_reveal_tx_hex || !base_deposit_send_to_op_return_tx_hex) {
      throw new Error('Base deposit transaction hexes are missing')
    }
    extra_tx_hexes.push(base_deposit_commit_tx_hex, base_deposit_reveal_tx_hex, base_deposit_send_to_op_return_tx_hex)

    extra_utxos.push(
      {
        utxo: `${base_deposit_send_to_op_return_txid}:1`,
        value: dummy_utxo_value,
        script_type: utxo_output_type_from_outputScript(payer_wallet.outputScript, network),
        wallet: payer_wallet,
      },
    )
  }
  if (needs_allowance) {
    if (!allowance_commit_tx_hex || !allowance_reveal_tx_hex || !allowance_send_to_op_return_tx_hex) {
      throw new Error('Allowance transaction hexes are missing')
    }
    extra_tx_hexes.push(allowance_commit_tx_hex, allowance_reveal_tx_hex, allowance_send_to_op_return_tx_hex)
  ...
  try {
    if (extra_tx_hexes.length > 0) {
      save_extra_utxos(extra_tx_hexes, null)
    }
    deposit_mint_res = await mint_with_extra_input_in_commit_all(deposit_inscription_details, extra_utxos, fee_rate, null, null, 0, true, signFn)
    ...
    save_extra_utxos(extra_tx_hexes, [deposit_inscription_id, deposit_satpoint])
    const target_wallet = new WalletInfo(true, Buffer.from(Script.encode(['OP_RETURN', Buff.str('BRC20PROG')], false)), null, null, null)
    send_to_opreturn_res = await send_inscription_to_op_return_with_extra_inputs_and_extra_output_all(deposit_inscription_id, [], target_wallet, 1, [], fee_rate, true, signFn)
  }
  finally {
    clear_extra_utxos()
  }
  ...
  const to_send_for_broadcast: BroadcastDepositOrderRequest = {
    ...
    approve_details: needs_allowance
      ? {
    ...
    base_deposit_details: use_base_available_balance_amt > 0n
      ? {
          commit_txid: base_deposit_commit_txid!,
          commit_txhex: base_deposit_commit_tx_hex!,
          reveal_txid: base_deposit_reveal_txid!,
          reveal_txhex: base_deposit_reveal_tx_hex!,
          send_to_opreturn_txid: base_deposit_send_to_op_return_txid!,
          send_to_opreturn_txhex: base_deposit_send_to_op_return_tx_hex!,
          secret: base_deposit_secret!,
          inscription_id: base_deposit_inscription_id!,
          fee_rate,
        }
      : null,
  }
...
export async function checkMinerFeesOfDepositOrder(
  ...
  fee_rate: number,
  create_allowance_if_needed: boolean = true,
): Promise<{ needs_approval: boolean, allowance_fees_total: number, base_deposit_fees_total: number, deposit_fees_total: number, fee_rate: number }> {
  // Get connected wallet
  const walletInfo = getWalletInfo()
  ...
  if (use_base_available_balance_amt > 0n) {
    ...
    const base_deposit_mint_res = await mint_all_check_fees(base_deposit_inscription_details, fee_rate, null, null, 0)
    ...
    try {
      ...
      send_to_opreturn_res = await send_inscription_to_op_return_with_extra_inputs_and_extra_output_fee_rate(base_deposit_inscription_id, [], target_wallet, 1, extra_outputs, fee_rate)
    }
    finally {
      clear_extra_utxos()
    }
  ...
  if (needs_allowance) {
    ...
    try {
      save_extra_utxos(extra_txhexes_for_allowance, null)
      const allowance_mint_res = await mint_all_check_fees(allowance_inscription_details, fee_rate, null, null, 0)
    ...
    try {
      ...
      send_to_opreturn_res = await send_inscription_to_op_return_with_extra_inputs_and_extra_output_fee_rate(allowance_inscription_id, [], target_wallet, 1, extra_outputs, fee_rate)
    }
    finally {
      clear_extra_utxos()
    }
  ...
  try {
    ...
    const deposit_mint_res = await mint_with_extra_input_in_commit_fee_rate(deposit_inscription_details, extra_utxos, fee_rate, null, null, 0)
    ...
    save_extra_utxos(extra_tx_hexes, [deposit_inscription_id, deposit_satpoint])
    const target_wallet = new WalletInfo(true, Buffer.from(Script.encode(['OP_RETURN', Buff.str('BRC20PROG')], false)), null, null, null)
    send_to_opreturn_res = await send_inscription_to_op_return_with_extra_inputs_and_extra_output_fee_rate(deposit_inscription_id, [], target_wallet, 1, [], fee_rate)
  }
  finally {
    clear_extra_utxos()
  }
...
async function broadcastWrapOrder(to_send: BroadcastWrapOrderRequest): Promise<BroadcastWrapOrderResponse> {
...
async function estimateGasWrapOrder(to_send: BroadcastWrapOrderRequest): Promise<EstimateGasWrapOrderResponse> {
...
export async function createAndBroadcastWrapOrder(
  ...
  const wrap_call_data = ethers.AbiCoder.defaultAbiCoder().encode(
    ...
    [
      l1_contract_address,
      Buffer.from(call_data.slice(2), 'hex'),
    ],
  )
  ...
  const deposit_content = Buff.str(`{"p":"brc20-prog","op":"c","c":"${swap_info.wbtc_address}","b":"${deposit_calldata_compressed}"}`)
  let estimated_gas = 0
  for (let i = 0; i < 5; i++) {
    const deposit_content_gas_allocation = deposit_content.length * GAS_PER_BYTE
    const needed_padding = estimated_gas > deposit_content_gas_allocation ? Math.ceil((estimated_gas - deposit_content_gas_allocation) / GAS_PER_BYTE) : 0
    const padded_deposit_content = Buff.from(Buffer.concat([deposit_content, Buff.str(' '.repeat(needed_padding))]))
    const deposit_inscription_details = new InscriptionDetails(
      Buff.str('text/plain'),
      null,
      null,
    ...
    const deposit_mint_res = await mint_with_extra_input_in_commit_all(deposit_inscription_details, [], fee_rate, null, null, 0, true, signFn)
    ...
    let send_to_opreturn_res = null
    try {
      save_extra_utxos([deposit_commit_tx_hex, deposit_reveal_tx_hex], [deposit_inscription_id, deposit_satpoint])

      const target_wallet = new WalletInfo(true, Buffer.from(Script.encode(['OP_RETURN', Buff.str('BRC20PROG')], false)), null, null, null)
      const extra_output_utxos = [ // send BTC to WBTC handler
        {
          wallet: new WalletInfo(false, null, swap_info.wbtc_handler_address, null, null),
          value: Number.parseInt(btc_amount.toString()),
        },
      ]
      send_to_opreturn_res = await send_inscription_to_op_return_with_extra_inputs_and_extra_output_all(deposit_inscription_id, [], target_wallet, 1, extra_output_utxos, fee_rate, true, signFn)
    }
    finally {
      clear_extra_utxos()
    }
    ...
    const estimate_padded = Math.max(estimate_gas_res.estimated_gas + 100000, estimate_gas_res.estimated_gas * 1.2)
...
export async function checkMinerFeesOfWrapOrder(
  ...
  const deposit_calldata_compressed = await compressSmartContractData(wrap_call_data_full)
  const deposit_content = Buff.str(`{"p":"brc20-prog","op":"c","c":"${swapInfo.wbtc_address}","b":"${deposit_calldata_compressed}"}`)
  const deposit_inscription_details = new InscriptionDetails(
    Buff.str('text/plain'),
    null,
    null,
  ...
  let deposit_fees_total = 0
  const deposit_mint_res = await mint_with_extra_input_in_commit_fee_rate(deposit_inscription_details, [], feeRate, null, null, 0)
  const deposit_commit_tx_hex = deposit_mint_res.unsigned_commit_tx_hex
  const deposit_reveal_tx_hex = deposit_mint_res.signed_reveal_tx_hex
  const deposit_inscription_id = deposit_mint_res.inscription_id
  const deposit_satpoint = `${deposit_inscription_id.split('i')[0]}:0:0`
  ...
  try {
    ...
    send_to_opreturn_res = await send_inscription_to_op_return_with_extra_inputs_and_extra_output_fee_rate(deposit_inscription_id, [], target_wallet, 1, extra_output_utxos, feeRate)
  }
  finally {
    clear_extra_utxos()
  }
...
async function getAddLiquiditySignature(order_params: AddLiquiditySignatureRequest): Promise<string> {
...
async function getAddLiquidityBLSSignature(params: AddLiquidityBLSSignatureRequest): Promise<string> {
  params.pubkey = params.pubkey.startsWith('0x') ? params.pubkey.slice(2) : params.pubkey
...
async function sendAddLiquidityOrder(to_send: AddLiquidityOrderRequest): Promise<AddLiquidityOrderResponse> {
...
async function getRemoveLiquiditySignature(order_params: RemoveLiquiditySignatureRequest): Promise<string> {
  order_params.token1_addr = order_params.token1_addr.toLowerCase()
  order_params.token2_addr = order_params.token2_addr.toLowerCase()
...
async function getRemoveLiquidityBLSSignature(params: RemoveLiquidityBLSSignatureRequest): Promise<string> {
  params.pubkey = params.pubkey.startsWith('0x') ? params.pubkey.slice(2) : params.pubkey
...
async function sendRemoveLiquidityOrder(to_send: RemoveLiquidityOrderRequest): Promise<RemoveLiquidityOrderResponse> {
...
export async function getSwapResult(
  ...
  const quoted_price = token_in_addr.toLowerCase() === swap_info.wbtc_address.toLowerCase()
  ...
  return { amount_out: result.amounts[1]!, quoted_price: quoted_price_number, price_impact_bps: result.price_impact_bps }
}
...
export async function getSwap2Result(
  ...
  return { amount_in: result.amounts[0]!, quoted_price: quoted_price_number, price_impact_bps: result.price_impact_bps }
}
...
async function getWithdrawOrderSignature(order_params: WithdrawOrderSignatureRequest): Promise<string> {
...
export async function getUnwrapResult(
  pkscript: string,
  amt: bigint,
): Promise<{ amt: bigint }> {
  const swap_info = await getSwapInfo()
