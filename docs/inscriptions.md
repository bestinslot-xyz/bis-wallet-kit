...
The `mint` namespace creates ordinal inscriptions and estimates their fees. The top-level builders
turn content into an `InscriptionDetails`.
...
Set `dryRun: true` to get the same shape back without touching the network — handy for previews and
tests.
...
(`{ paymentAddress, paymentAmount }`) when payment should come from a separate address.
